...
class NerDataProcessor(object):
    ...
    def read_data(self,path,is_training_data=True): # 把字符串组合成序列样本，标签也组合成序列，每个序列就是一个列表。如果是训练集，设定为true
        """
        数据格式如下（分隔符为空格）：
        便 B_disease
        秘 I_disease
        ...
        sentence = []
        labels = []
        split_pattern = re.compile(r'[；;。，、？！\.\?,! ]')     # 保证每一个训练样本的长度不超过max_len，对每一个句子进行切分
        with open(path,'r',encoding = 'utf8') as f:
            for line in f.readlines():
                #每行为一个字符和其tag，中间用tab或者空格隔开
                line = line.strip().split()
                if(not line or len(line) < 2):  # 如果是空行或者字符数量小于2，说明一句话结束了
                    X.append(sentence.copy())   # 就将它作为一个样本，append进去
                    y.append(labels.copy())
                    sentence.clear()
                    labels.clear()
                    continue
                word, tag = line[0], line[1]    # 切分成了一个word，一个tag
        ...
        if is_training_data:    # 如果是训练集，就要进行一系列的赋值，从训练集中提取
    ...
    def encode(self,X,y):
        ...
        将标签进行 one-hot"""
        X = [[self.word2id.get(word,1) for word in x] for x in X ]  # 训练集进行映射
        X = pad_sequences(X,maxlen=self.max_len,value=0)
        y = [[self.tag2id.get(tag,0) for tag in t] for t in y ]     # 标签进行映射
        y = pad_sequences(y,maxlen=self.max_len,value=0)            # 使用0作为padding值

        def label_to_one_hot(index: []):    # 对每一个token的语义向量进行多分类，对多分类的标签进行one-hot处理
            data = []
            for line in index:
                data_line = []
